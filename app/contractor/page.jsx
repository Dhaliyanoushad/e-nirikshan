  ...
  async function createProject(e) {
    ...
    try {
      ...
      if (pdfFile) {
        ...
        formData.append("start_date", form.start_date);
        formData.append("end_date", form.end_date);
        await fetch("/api/create-project", { method: "POST", body: formData });
      }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                  ...
                  {selectedProject.latest_photo && (
                    <img src={selectedProject.latest_photo.url} className="w-full h-40 object-cover rounded-xl mb-4" alt="Site" />
                  )}
                  <input type="file" onChange={(e) => handlePhotoSelect(e, selectedProject.project_id)} className="text-xs mb-3" />
                  <button onClick={() => uploadPhoto(selectedProject.project_id)} className="w-full bg-green-600 text-white py-2 rounded-xl font-bold text-sm">Update AI Photo</button>
                </div>
              </div>
            </div>
          </div>
